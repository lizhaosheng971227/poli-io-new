...
RUN_BUILD_BACKBONE = False
RUN_FINALIZE_WITH_PARTY_CHAR = True
PARTY_CHAR_PATH = OUTPUT_DIR / "party_char_v3_campaignavg_repaired.dta"  # edit later if needed
...
def cast_stata_friendly(df: pd.DataFrame) -> pd.DataFrame:
    """Coerce columns into dtypes pandas can export to Stata reliably."""
    out = df.copy()

    # Known numeric columns that may otherwise become object due to mixed scalars / pd.NA.
    numeric_like = {
        "election_year", "mpd_party_id", "parfam", "share_mpd", "share_key",
        "n_alias_local", "n_alias_eng", "ess_rows_all", "ess_rows_vote12",
        "ess_weight_all", "ess_weight_vote12", "n_markets_vote12", "n_regions_vote12",
        "has_any_manual_flag", "manual", "vote", "anweight", "pspwght", "pweight",
        "female", "educ_cat", "age", "college", "lrscale", "young", "middle",
        "old", "boomer", "age_at_election", "dob", "agent_ids", "candidate_age",
        "candidate_gender", "candidate_gender_male1"
    }
    ...
    for c in out.columns:
        ...
        if pd.api.types.is_object_dtype(s) or pd.api.types.is_string_dtype(s):
            non_null = s.dropna()
            # All-null object arrays are not exportable; force to empty strings.
            if len(non_null) == 0:
                out[c] = pd.Series([""] * len(s), index=s.index, dtype="string")
                continue
            # If everything non-missing looks numeric, export as numeric.
...
def write_stata_safe(df: pd.DataFrame, path: Path, version: int = 118) -> None:
    out = cast_stata_friendly(df)
    out.to_stata(path, write_index=False, version=version)
...
def load_and_harmonize(validated_path: Path) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    if per_cols:
        per_block = df[per_cols].apply(to_num)
        out = pd.concat([out, per_block], axis=1)
    meta["n_per_cols"] = len(per_cols)

    # Light defragmentation before later inserts.
    out = out.copy()
...
def finalize_with_party_char(auto_path: Path, party_char_path: Path, out_path: Path) -> pd.DataFrame:
    ...
    write_stata_safe(out, out_path, version=118)
...
def main() -> None:
    ...
    if RUN_BUILD_BACKBONE:
        ...
        write_stata_safe(auto, AUTO_OUT, version=118)
        write_stata_safe(demo, DEMO_OUT, version=118)
        pe.to_csv(PARTY_ELECTION_CSV, index=False)
        write_stata_safe(pe, PARTY_ELECTION_DTA, version=118)
        seed.to_csv(AI_SEED_CSV, index=False)
        write_stata_safe(seed, AI_SEED_DTA, version=118)
